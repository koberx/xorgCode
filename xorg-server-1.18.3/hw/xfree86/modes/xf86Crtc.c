                 ...
                 const xf86OutputFuncsRec * funcs, const char *name)
{
    ...
    ErrorF("xf86OutputCreate num_output %d (already++)\n", xf86_config->num_output);
    return output;
}
...
xf86ProbeOutputModes(ScrnInfoPtr scrn, int maxX, int maxY)
{
    ...
    ErrorF("lks xf86ProbeOutputModes config->num_output %d\n", config->num_output);
    ...
    for (o = 0; o < config->num_output; o++) {
        ...
        Bool debug_modes = 1; //config->debug_modes || xf86Initialising;
        enum det_monrec_source sync_source = sync_default;

        ErrorF("lks get mode[%d] begin\n", o);
        ...
        ErrorF("lks output->status %d xf86ReturnOptValBool %d\n",
               output->status, xf86ReturnOptValBool(output->options, OPTION_ENABLE, FALSE));
        if (output->status == XF86OutputStatusDisconnected &&
            !xf86ReturnOptValBool(output->options, OPTION_ENABLE, FALSE)) {
            ErrorF("lks xf86ProbeOutputModes call xf86OutputSetEDID\n");
        ...
        if (conf_monitor) {
            ...
            ErrorF("lks get xf86GetMonitorModes[%d] %dx%d\n", o,
                   config_modes->HDisplay,config_modes->VDisplay);
        }

        output_modes = (*output->funcs->get_modes) (output);

        ErrorF("lks get output_modes %p after get_modes\n", output_modes);
        if(output_modes) {
            ErrorF("lks output_modes info: %dx%d\n", output_modes->HDisplay, output_modes->VDisplay);
        }
        /*
        ...
        if (!mon_rec.nHsync || !mon_rec.nVrefresh) {
            ...
            ErrorF("lks get output_modes %p after GuessRangeFromModes\n", output_modes);
        }
        /*
        ...
        if (sync_source == sync_config) {
            ErrorF("lks sync_source==sync_config\n");
            /*
        ...
        ErrorF("lks xf86ProbeOutputModes final config_modes %p output_modes %p default_modes %p\n",
               config_modes, output_modes, default_modes);
        ...
        ErrorF("lks probed modes modified by xf86SortModes\n");
...
xf86SetScrnInfoModes(ScrnInfoPtr scrn)
{
    ...
    if (!scrn->modes) {
        ErrorF("lks xf86SetScrnInfoModes call xf86ModesAdd\n");
