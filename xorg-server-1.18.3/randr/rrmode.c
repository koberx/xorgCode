...
RRModeCreate(xRRModeInfo * modeInfo, const char *name, ScreenPtr userScreen)
{
	...
	ErrorF("chenli RRModeCreate num_modes=%d, mode->name=%s\n", num_modes, mode->name);
    /*
...
RRModeGet(xRRModeInfo * modeInfo, const char *name)
{
	...
	ErrorF("chenli RRModeGet num_modes=%d, name=%s\n", num_modes, name);
                 ...
                 xRRModeInfo * modeInfo, const char *name, int *error)
{
	...
	ErrorF("chenli RRModeCreateUser num_modes=%d, name=%s\n", num_modes, name);
...
RRModesForScreen(ScreenPtr pScreen, int *num_ret)
{
	...
	ErrorF("chenli RRModesForScreen num_modes=%d\n", num_modes);
    /*
    ...
    for (o = 0; o < pScrPriv->numOutputs; o++) {
        ...
        for (m = 0; m < output->numModes + output->numUserModes; m++) {
            ...
            {
            	ErrorF("chenli RRModesForScreen Add modes from all outputs output->numModes=%d, output->numUserModes=%d, num_screen_modes=%d, name=%s\n", output->numModes, output->numUserModes, num_screen_modes, mode->name);
				printf("chenli RRModesForScreen Add modes from all outputs num_screen_modes=%d, name=%s\n", num_screen_modes, mode->name);
    ...
    for (c = 0; c < pScrPriv->numCrtcs; c++) {
        ...
        if (n == num_screen_modes)
        {
        	ErrorF("chenli RRModesForScreen Add modes from all crtcs num_screen_modes=%d, name=%s\n", num_screen_modes, mode->name);
			printf("chenli RRModesForScreen Add modes from all crtcs num_screen_modes=%d, name=%s\n", num_screen_modes, mode->name);
            screen_modes[num_screen_modes++] = mode;
        }
    }
    /*
    ...
    for (m = 0; m < num_modes; m++) {
        ...
        if (n == num_screen_modes)
        {
       		ErrorF("chenli RRModesForScreen Add all user modes for this screen num_screen_modes=%d, name=%s\n", num_screen_modes, mode->name);
			printf("chenli RRModesForScreen Add all user modes for this screen num_screen_modes=%d, name=%s\n", num_screen_modes, mode->name);
            screen_modes[num_screen_modes++] = mode;
        }
    }
...
ProcRRCreateMode(ClientPtr client)
{
	...
	ErrorF("chenli ProcRRCreateMode call RRModeCreateUserm, name=%s\n", name);
